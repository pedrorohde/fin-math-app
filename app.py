...
amort = st.selectbox("Sistema de Amortização",
    options=[
        "PRICE",
        "SAC",
        "SAM",
...
P = st.number_input('Valor do Principal ($)', value=1000.0, min_value=0.01, step=1.0, format="%.2f")
r = st.number_input('Taxa de Juros (%)', value=1.0, min_value=0., step=0.5, format="%f")/100
N = st.number_input('Número de Períodos', value=12, min_value=1)
